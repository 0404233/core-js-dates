...
function isDateInPeriod(date, period) {
  ...
  return nowDate >= periodStart && nowDate <= periodEnd;
}
...
function getWeekNumberByDate(date) {
  ...
  const startWeek = startYear.getDate();
  ...
  const result = Math.ceil(totalDays / 7);
  return result;
}
...
function getNextFridayThe13th(date) {
  for (let i = date.getMonth(); i < 12; i += 1) {
    const nowDate = new Date(date.getFullYear(), i, 13);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startDate = new Date(
    `${datePartsStart[2]}-${datePartsStart[1]}-${datePartsStart[0]}`
  );
  const endDate = new Date(
    `${datePartsEnd[2]}-${datePartsEnd[1]}-${datePartsEnd[0]}`
  );
